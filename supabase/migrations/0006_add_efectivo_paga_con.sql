-- Denominación de billete con la que el cliente pagará en efectivo, para
-- que el restaurante/repartidor lleve el cambio exacto.

alter table public.pedidos add column efectivo_paga_con integer;

-- "not valid": ya existen pedidos en efectivo anteriores a esta columna,
-- todos con efectivo_paga_con nulo. Se exige solo hacia adelante, sin
-- validar retroactivamente el historial.
alter table public.pedidos add constraint efectivo_paga_con_requerido
  check (metodo_pago <> 'efectivo' or efectivo_paga_con is not null) not valid;

alter table public.pedidos add constraint efectivo_paga_con_positivo
  check (efectivo_paga_con is null or efectivo_paga_con > 0);

-- Redefinición completa de crear_pedido (0004) agregando la validación de
-- que el billete elegido alcance para cubrir el total real calculado aquí
-- mismo — nunca se confía en un total enviado por el cliente.
...
begin
  ...
  if v_metodo_pago = 'efectivo' then
    ...
    elsif v_efectivo_paga_con < v_subtotal + v_costo_domicilio then
  ...
  insert into public.pedidos (
    ...
    subtotal, costo_domicilio, total, efectivo_paga_con
  ) values (
    ...
    v_subtotal, v_costo_domicilio, v_subtotal + v_costo_domicilio, v_efectivo_paga_con
